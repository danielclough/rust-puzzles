...
pub fn exec(menu_config: &MenuConfig, quiz_list_state: &mut ListState, result_list_state: &mut ListState) {
    match menu_config.active_item {
        ...
        MenuItem::Results => {
            if let Some(selected) = result_list_state.selected() {
                let n_items = read::read_results().expect("can fetch quiz list").len();
                if selected > 0 {
                    result_list_state.select(Some(selected - 1));
                } else {
                    result_list_state.select(Some(n_items - 1));
                }
            }
        }
        _ => {}
